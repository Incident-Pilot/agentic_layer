...
class ApplicationInvestigationAgent:
    ...
    async def __call__(self, state: AgentState) -> dict:
        ...
        ctx_payload = {
            ...
            "avoid_angle": rejected[-1].root_cause if rejected else None,
        }
        system = system_header("investigate-tools", _ROLE_DESCRIPTION) + "\n\n" + json_block("CONTEXT", ctx_payload)
        ...
        avoid_note = f" Avoiding previously rejected angle: '{ctx_payload['avoid_angle']}'." if ctx_payload["avoid_angle"] else ""
        reasoning = (
            ...
            f"(plus {len(new_evidence) - len(tool_evidence)} seeded from incident context)." + avoid_note
        )
